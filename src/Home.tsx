...
import { KekeCallRouter } from "./components/KekeCallRouter";
...
// which fetches its own post from Supabase — Home doesn't need to know
// about post routes at all.
//
// This assumes a `broadcasts` table: id, title, message, audiences,
// image (nullable), created_at. Point the fetch below at whatever
// table/view actually stores sent broadcasts.
...
const BUSINESSES: Business[] = [
  ...
  },
  {
    ...
    deal: true,
    verifiedSales: [
  { id: "nl-vs1", image: "nanzzrep1.jpeg" },
  { id: "nl-vs2", image: "nanzzrep2.jpeg" },
  ],
  },
   {
    id: "47",
  ...
  },
 {
    ...
    accent: "from-cyan-400/25 to-emerald-400/15",
    logo: "/khalcarelogo.jpeg",
    image: "/khalcare1.jpeg",
...
function Hero() {
  return (
    <section className="relative mx-auto max-w-7xl px-6 pt-20 pb-10 sm:pt-28">
     ...
     <p className="mt-6 max-w-xl text-base sm:text-lg text-muted-foreground leading-relaxed" style={{ marginBottom: "2.5rem" }}>
  A unified digital platform connecting students to verified campus vendors, farm produce, and safety services — built for Federal University Dutse.
</p>
  ...
  return (
    <section className="mx-auto max-w-7xl px-6">
      <div className="reveal glass-card rounded-2xl p-6 sm:p-8">
        <h2 className="text-lg sm:text-xl font-display font-semibold tracking-tight">Get Campus Alerts</h2>
        <p className="mt-2 text-sm text-muted-foreground max-w-lg">
          Get a text the moment a campus broadcast goes out — new vendors, farm produce drops, safety notices.
        </p>
...
// ── List Your Business CTA (ghost button, full width) ─────────────────────────
function ListYourBusinessCTA() {
  const href = `https://wa.me/2347044389234?text=${encodeURIComponent("Hi EMBIEM! I'd like to list my business on FUD Hub.")}`;
  return (
    <section className="mx-auto max-w-7xl px-6" style={{ marginTop: "20px", marginBottom: "30px" }}>
      <a
        href={href}
        target="_blank"
        ...
        style={{
          ...
          width: "100%", padding: "1.15rem", borderRadius: 18,
          border: "1.5px solid color-mix(in oklab, var(--emerald-glow) 55%, transparent)",
          background: "transparent", color: "var(--emerald-bright)",
          fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem",
          letterSpacing: "0.01em", textDecoration: "none", cursor: "pointer",
          transition: "background 0.25s ease, box-shadow 0.25s ease",
        }}
        onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.background = "color-mix(in oklab, var(--emerald-glow) 8%, transparent)"; (e.currentTarget as HTMLAnchorElement).style.boxShadow = "0 0 40px -12px var(--emerald-glow)"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.background = "transparent"; (e.currentTarget as HTMLAnchorElement).style.boxShadow = "none"; }}
      >
        List Your Business on FUD Hub
...
const LODGES: Lodge[] = [
  {
    ...
    images: [
      ...
      { url: "/lodges/ExclusivegymT.jpeg", label: "WC" },
      { url: "/lodges/Exclusivegym.jpeg", label: "Yard" },
  ],
      ...
      >
        ...
        <div style={{ position: "relative", aspectRatio: "4/3", flexShrink: 0, background: "oklch(0.12 0.015 250)", overflow: "hidden" }}>
          <img
            ...
            style={{ width: "100%", height: "100%", objectFit: "contain", animation: "fadeIn 0.22s ease" }}
...
function LodgeCard({ lodge, index, onOpen }: { lodge: Lodge; index: number; onOpen: () => void }) {
  ...
  return (
    ...
    >
      {/* Image + overlay pills */}
      <div
        style={{ position: "relative", aspectRatio: "3/3", background: "oklch(0.13 0.015 250)", overflow: "hidden", flexShrink: 0 }}
      >
        <img
          key={activeImg}
          src={lodge.images[activeImg].url}
  ...
  return (
    <>
      <section className="mx-auto max-w-7xl px-6 py-12 sm:py-16">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.70rem" }}>
          {LODGES.map((lodge, i) => (
...
function Stats() {
  return (
    <section id="stats" className="mx-auto max-w-7xl px-6 pb-24">
      <div className="reveal glass-card p-8 sm:p-10 grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
        {STATS.map((s) => (
          <div key={s.v}>
...
console.error('Upload error:', error);
  ...
  const handleSend = () => {
    ...
    onSend(title.trim(), message.trim(), selected, imagePreview || undefined);
    setTitle("");
    setMessage("");
    setSelected([]);
  ...
  return (
    <div className="reveal glass-card rounded-2xl p-6 sm:p-8" style={{ marginBottom: "2.5rem" }}>
      ...
      <p className="text-sm text-muted-foreground mb-6">Send a live announcement — it shows up in every student's Campus Updates menu.</p>

      <label style={labelStyle}>Title</label>
      <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. AI, Data Science, 18 other programmes ready for NUC verification" style={{ ...inputStyle, marginBottom: "1.5rem" }} />
...
function FarmManagerPortal({ setAgriProducts, addToast, onLock, onSendBroadcast }: {
  ...
  onLock: () => void;
  onSendBroadcast: (title: string, message: string, audiences: Audience[], image?: string) => void;
}) {
...
function Footer() {
  return (
    <footer className="w-full border-t border-white/5 bg-black/20 backdrop-blur-md" style={{ marginTop: "2rem" }}>
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
          <div className="flex items-center gap-3">
            <div className="relative h-10 w-10 rounded-xl overflow-hidden border border-white/10 flex-shrink-0">
              <img src="/embiem-logo.png" alt="EMBIEM" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-[0.18em] text-emerald-bright">Built by EMBIEM</div>
              <p className="text-sm text-white/80 mt-1 max-w-sm">
                Built for FUD students, by FUD students — from onboarding vendors to keeping the Hub running.
              </p>
            </div>
          </div>
          <a href={`https://wa.me/2347044389234?text=${encodeURIComponent("Hi EMBIEM! I'd like to list my business on FUD Hub.")}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-[0_0_30px_-5px_var(--emerald-glow)] hover:shadow-[0_0_50px_-2px_var(--emerald-glow)] transition-shadow flex-shrink-0">
            Add your business →
          </a>
        </div>
...
export default function Home() {
  ...
  // `broadcasts`: id, message, created_at, plus title/image (added via
  // migration). Audiences live in a separate `broadcast_audiences` table
  // (broadcast_id, audience) — joined and flattened below.
  useEffect(() => {
    const fetchBroadcasts = async () => {
      ...
      if (data) {
        setBroadcasts(
          data.map((b: any) => ({
            id: b.id,
            title: b.title || b.message.slice(0, 60),
            message: b.message,
            audiences: (b.broadcast_audiences || []).map((a: any) => a.audience),
  ...
  const handleSendBroadcast = async (title: string, message: string, audiences: Audience[], image?: string) => {
    const newBroadcast: Broadcast = { id: `bc-${Date.now()}`, title, message, audiences, createdAt: Date.now(), image };
    setBroadcasts((prev) => [newBroadcast, ...prev]);

    const { data, error } = await supabase.functions.invoke('send-broadcast', {
      body: { title, message, audiences, image },
    });
...
/>
      ...
      <CampusUpdates open={showUpdates} onClose={() => setShowUpdates(false)} broadcasts={broadcasts} />
